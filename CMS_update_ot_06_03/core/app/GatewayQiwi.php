...
class GatewayQiwi extends Model {
	...
	protected $table = "gateway_p2pqiwi";
	protected $guarded = [
		'id',
		'gateway_id'
	];
	...
	public function payment_handler($request) {
		...
		if(@$_SERVER['HTTP_X_API_SIGNATURE_SHA256'] == hash_hmac('sha256', $invoice, $this->gateway()->key_secret)) {
			...
			if(@$user->inviter_id && @$this->general->invite_percent) {
				...
				$bonus_user->balance = number_format($bonus_user->balance + $bonus_invt, 2, '.', '');
				...
				GatewayTransact::create([
					...
					'amount' => $bonus_invt,
					'user_balance' => $bonus_user->balance,
					'charge' => null,
					'type' => '3',
			...
			$payment->money = $money;
			$payment->save();
			$user->balance = number_format($user->balance + $money_bonus, 2, '.', '');
			$user->save();
			GatewayTransact::create([
				'user_id' => $user->id,
				'gateway' => 'Qiwi',
	...
	public function payment_redirect($request) {
		$this->qiwi_api = new \Qiwi\Api\BillPayments($this->gateway()->key_secret);
		$user = Auth::user();
        ...
        $fields = array(
            ...
            'successUrl'            => route('user.dashboard'),
            'account' 				=> @$user->id,
            'email' 				=> @$user->email,
