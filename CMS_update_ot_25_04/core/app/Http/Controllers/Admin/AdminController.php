...
class AdminController extends Controller {
    ...
    public function dashboard() {
        ...
        $data['payments_count'] = GatewayPaylogs::count('id');

        // Получаем новых пользователей
        $data['users'] = User::orderByRaw('id DESC')->limit(10)
        ->get();
        // Получаем платежи
        $data['users_payments'] = GatewayPaylogs::orderByRaw('id DESC')->limit(10)
        ->get();
        // Получаем платежи
        $data['success_payments'] = GatewayPaylogs::where('status', 1)->orderByRaw('id DESC')->limit(10)
        ->get();
    ...
    public function gatewaysListSave(Request $request) {
        ...
        if(@$gateway_id) {
            ...
            if(@$gateway_id == 3) {
                ...
                $update = GatewayQiwi::where('id', '=', 1)->update(@$excp + ['status' => $status]);
            }
            if(@$update) {
                $data = array(
                    'type' => 'success',
